...
Now, imagine that we have regions that do not have a defined strand (e.g. most ChIP-seq peaks). In this case, the `col_by_strand` is internally converted to FALSE and the regions are colored by region set (i.e. elements in `regions_list`). Look at this example with only one region set whose strand values are converted to ".".
